...
class vipPersonalAchievementList extends Component {
  constructor () {
    super()
  ...
  getTitle () {
    if( this.props.location.state.tradeType ===3) {
      return '本月用户快捷收款交易量 (元)'
      
    }
    if( this.props.location.state.tradeType === 4) {
      return  '本月用户快捷还款交易量 (元)'
    }
  }
      ...
      <div>
        <div className="headWrap">
          ...
          <div className="searchBox">
            ...
            <input placeholder='请输入查找的用户姓名' className='searchInput' value={this.state.organName} onChange={this.searchContent} type="search"/>
                        ...
                        <Link key={index} to={{
                          ...
                          <li key={index}>
                            <div className={this.showHighlight(index)}>
                              {this.getPrefix(index + 1)}{ele.userName}
                            </div>
...
export default vipPersonalAchievementList