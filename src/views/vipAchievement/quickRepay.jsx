...
class quickRepay extends Component {
  constructor (props) {
    super(props)
    this.state = {
      current:0,
  ...
  async componentWillMount () {
    console.log(this.props.location.state.tradeType)
    console.log(this.state.teamTrade.tradeType)
          ...
          <div className="achievementHead">
            <ul className='types-title'>
              {this.state.types.map( (ele, index) =>{
                return (
                  <li key={index} onClick={() => this.getTypesIndex(index)} 
                    className={this.typesNav(index)}>{ele}</li>
                )
              })}
            </ul>
          </div>
