...
import SettlementRules from '../partnerAchievement/settlementRules'
...
class teamAchievement extends Component {
  ...
  getTitle() {
    if (this.props.teamTrade.tradeType === 3) {
      return '本月VIP团队收款交易量(元)'
    }
    if (this.props.teamTrade.tradeType === 4) {
      return '本月VIP团队还款交易量(元)'
    }
  }
  componentWillMount() {}
  render() {
    return (
      <div className="achievementWrap">
        <div className="totalTransaction">
          <Link
            to={{
              pathname: '/vipTeamAchievementList',
          ...
          >
            <div className="tradingVolume">
              ...
              <div className="excellentPerson">
                <p>
                  <span style={{ fontSize: '.12rem' }}>达成亿元 (人)：</span>
                  {this.props.teamTrade.partnerIndexSubEntity.hundredOfMillionPeoples}
                </p>
                <div style={{ width: '1px', height: '.15rem', backgroundColor: '#4288ff' }} />
                <div>
                  <span style={{ fontSize: '.12rem' }}>达成两千万 (人)：</span>
                  {this.props.teamTrade.partnerIndexSubEntity.twentyMillionPeoples}
                </div>
              </div>
            </div>
          </Link>
        ...
        <div className="achievementReward">
          <div className="rewards">
            ...
            <p className="settlementRules">
              <span onClick={() => this.popRules()}>结算规则</span>
            </p>
          </div>
        </div>
        <SettlementRules
          show={this.state.sendShow}
          getHide={() => {
            this.getHide()
          }}
        />
