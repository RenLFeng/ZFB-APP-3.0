...
class teamAchievementList extends Component {
    ...
    if (isBillion === 1) {
      return (
        <img
          ...
          src={require('../../assets/img/achievementBillon.png')}
          alt=""
        />
      )
    }
    if (isMust === 1) {
      return (
        <img
          ...
          src={require('../../assets/img/achievementTwentyMillon.png')}
          alt=""
        />
      )
    }
  ...
  getTitle() {
    if (this.props.location.state.tradeType === 1) {
      return '本月合伙人设备收款交易量 (元)'
    }
    if (this.props.location.state.tradeType === 2) {
      return '本月合伙人设备还款交易量 (元)'
    }
  }
              ...
              {this.state.repaymentListData.map((ele, index) => {
                return (
                  <Link
                    key={index}
                    to={{
                      pathname: '/partnerDetail',
                      state: { organType: 2, organId: ele.organId }
                    }}
                  >
                    <li key={index}>
                      <div className={this.showHighlight(index)}>
                        ...
                        {ele.userName}
                        {this.getAfterfix(ele.isBillion, ele.isMust)}
                      </div>
...
export default teamAchievementList
